...
2. 登录（GitHub 账号）
3. 左侧栏目：**站点设置 / 首页 / 关于我们 / 各产品页 / 博客文章**，选中即改：文字、图片（上传新图）、产品、FAQ、联系方式、发布新闻/公告
4. 点 **保存**（Publish）→ 自动提交 GitHub → 1-3 分钟后线上自动更新

> **博客文章**：`/admin/` → 博客文章 → 新建。填 文件标识(英文小写)/标题/日期/封面/摘要/正文（段落、小标题）→ 保存即上线（列表页 `/blog.html`）；不想上线可勾选「草稿」。文章自动进入 sitemap。
