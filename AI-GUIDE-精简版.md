...
**About 的 `blocks[]` 块类型**：`text` / `image` / `textImg`（图文组合，含 `direction`/`ratio`/`imgAlign`/`images[]`/`carousel{}`）/ `clients` / `faq` / `timeline`（年份大事记）/ `marquee`（无缝滚动横幅，**2026-09-23 起可多张图**）。
...
> ⚠️ **`marquee`**：`images[]`（**可多张**，每项 `{image,imageAlt}`；2026-09-23 起，旧 `image` 单张仍兼容）；`duration` = **滚动一圈的秒数**（默认 45，越大越慢），**张数越多整圈越长**——想保持同样快慢要按张数把秒数调大（2 张 ≈ 90）；命名刻意不用 `interval`（语义不同）。
> ⚠️ **多图「无缝滚动」的三条硬规则**（2026-09-23）：① **整份序列复制一份**再做 `-50%`（**不是逐张复制**）；② **每张图后面都跟一个「缝」占位条、末尾那张也要** —— 它同时提供「两张图相接处」与「循环点」那道缝，也是 -50% 能对齐的前提；③ **占位条宽度必须随图片等比缩放**（**源图的缝是烘焙在图里的、会跟着缩放**）——现在是桌面 **13px** / 手机 **8px**。⚠️ **别用 flex `gap` 顶替**（N 张图只有 N-1 道 gap → 循环点错位）。`[§10.39、坑 #36]`
...
- **补充模块的 `direction`（文字/图片的位置，2026-09-21 新增）**：`textRight`（**默认** = 图左文右 = **改动前的版式**）/ `textTop`（文上图下）/ `textBottom`（图上文下）/ `textLeft`（文左图右）——**取值与图文区一致**；**走白名单校验，缺省 / 非法值一律退回 `textRight`**（老数据零变化）；手机（≤700px）一律单列、**左右两种自动退回「文字在上」**；**没传图片时方向不生效**（仍按原样「只显示文字并居中」）。`[§10.38]`
...
- **用户标注（红圈 + 文字）是最高优先级需求。**
- ⚠️ **但标注要先分清"是在标问题、还是在标正确"**：用户框住一处写「有缝隙」是在说"**这里对、我要这样**"，
  而不是"这里错"——**拿不准就问一句**。实况（2026-09-23）：**理解反了**，把用户想要的缝全删掉、当天返工改回来。
  `[§10.39、坑 #36]`
