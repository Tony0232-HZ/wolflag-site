...
| 🟡 P1 | 图片文件名含中文（**待办**） | 共 **12 个含中文** + `1.webp` 无意义名，完整清单见 §10.10 末。改文件名须同步 JSON 引用 + `scripts/extract.py` 的 md5→语义名映射表（见坑 #3 相关）。**建议并入「产品页内容做厚」一起改** |
...
媒体库仍有 **12 个含中文的文件名**（`a型展架-remax.jpg`、`car-flag-米黄背景.webp`、`banner-新横幅.webp`、`h-stake-网站图.webp`、`product横幅-.jpg`、`x展架08.jpg`、`太阳能-灯箱-网站图.webp`、`易拉宝02.webp`、`横幅-关于我们.webp`、`横幅-示意图.webp`、`水滴型旗子.jpg`、`水滴型沙滩旗02.webp`）+ **1 个无意义名** `1.webp`。
**Google 也会读文件名**，英文语义化文件名对图片搜索有帮助。**改文件名须同步更新内容 JSON 的引用 + 重建**，建议并入「产品页内容做厚」一起做（同一批文件改一次）。
