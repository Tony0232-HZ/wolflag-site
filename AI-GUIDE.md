...
**⑥ 后台字段：没有新增** —— 文字**写死在 `build.mjs`**。理由同 §10.27.7：
用户非程序员、"以后不改"、字段越少越省事。要改文字喊 Claude。
