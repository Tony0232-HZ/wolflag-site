...
| 导航 | 52px；菜单从 logo 旁锚定铺展：main=16px/#272e47/无active下划线、active=#4c6aff bold+2px 蓝下划线、gap 24px（2026-09-08 自 31px 收紧）、`white-space:nowrap` 强制多词菜单名单行（防折行）、汉堡断点 `@media (max-width:1200px)`（原 900px）；Contact Us 右缘 #4c6aff 圆角6 |
...
banner/通用卡(product-card)说明模块：品名(p-name)→属性表(.p-spec，Size/Material 可自由增删、对照 sg-spec)→宣传语(.p-sub 13px、pre-line)**（2026-09-08，desc/material/detail 换 specs+subtitle）；
feather 卡: 淡蓝边框 #d9e2f5 圆角10、说明模块=品名(f-title 18px/700)→属性表(f-spec，Size/Material 可自由增删、对照 sg-spec)→宣传语(f-sub 13px 灰、pre-line)**（2026-09-08 起 size/material/desc 换 specs+subtitle、移除 CTA）；
